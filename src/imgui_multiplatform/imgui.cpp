...
struct OcornutImguiContext {
...
#if USE_ENTRY
        for (int32_t ii = 0; ii < (int32_t) entry::Key::Count; ++ii) { m_keyMap[ii] = ImGuiKey_None; }
    ...
    void beginFrame(int32_t _mx, int32_t _my, uint8_t _button, int32_t _scroll, int _width, int _height, int _inputChar,
        ...
        ImGuiIO &io = ImGui::GetIO();
        if (_inputChar >= 0) { io.AddInputCharacter(_inputChar); }
        ...
        io.AddKeyEvent(ImGuiKey_ModShift,
                       0 != (modifiers & (entry::Modifier::LeftShift | entry::Modifier::RightShift)));
...
void imguiCreate(float _fontSize, bx::AllocatorI *_allocator) { s_ctx.create(_fontSize, _allocator); }
